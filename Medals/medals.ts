...
type groups = 'autoconfirmed' | 'user' | 'content-moderator' | 'threadmoderator' | 'emailconfirmed' | 'rollback' | 'sysop' | 'bureaucrat' | 'bot' | 'blocked'

var array = {
    containsAllEntriesOf: function (array1: any[], array2: any[]) {
    ...
    containsOneEntryOf: function (array1: any[], array2: any[]) {
        for (var i = 0, l = array2.length; i < l; i++) {
            if (array1.indexOf(array2[i]) > -1) {
                return true;
            }
    ...
    checkIfUserRepeats: function (users: User[]) {
        const repeatedUsers = [] as ([string, number, number])[]
...
function getPartialUsers (usernames: string[]) {
    ...
    function addPartialUsersWithOffset () {
        return new Promise<void>(function (resolve, reject) {
            ...
            wiki.get(params, true)
            .then((response: AxiosResponse<LooseObject>) => {
...
function getAllLocalUsers () {
    ...
    return new Promise<typeof users>(function (resolve, reject) {
...
class Page {
    ...
    areThereChanges (content: string) { return content !== this.content; }
    submitChanges (content: string) {
        if (this.areThereChanges(content)) {
            wiki.edit({
                title: this.name,
                text: content,
                summary: 'Automatic adding of new editors'
            }).then(function (data: LooseObject) {
                console.log(data);
            });
            return;
        }
...
function createPageFromTitle(title: string) {
    return new Promise<Page>(function (resolve, reject) {
        wiki.getRaw(title)
        .then(function(data){
...
class Medal {
    ...
    weight: number;
    type: string;
    private _minimumEdits: number;
    private _minimumPosts: number;
    private _requiredGroups: groups[];
    private _excludingGroups: groups[];
    private _users: User[] = [];
    ...
    meetsRequirements(user: User) {
        if (user.edits < this._minimumEdits) { return false; }
        if (user.posts < this._minimumPosts) { return false; }
        if (!array.containsAllEntriesOf(user.groups, this._requiredGroups)) { return false; }
        return true;
    }

    addIfMeetsRequirements(user: User) {
...
class User {
    ...
    constructor(name: string, id: number, edits: number, posts: number, groups: groups[]) {
        this.name = name;
        this.id = id;
        this.edits = edits;
        this.posts = posts;
    ...
    highestWeightMedalOfTypes (medalTypes: string[]) {
...
function createUserFromPartialUser(partialUser: PartialUser) {
    var userProperties = {
        ...partialUser
    } as User

    function parseNumber(edit: string | number) {
    ...
    return new Promise<User>(function (resolve, reject) {
        function getEditsAndPosts() {
            ...
            .then(function (response: AxiosResponse<LooseObject>) {
                ...
                resolve(new User(
                    userProperties.name,
                    userProperties.id,
                    userProperties.edits,
                    userProperties.posts,
...
function syncMedalsPageWithScript (users: User[]) {
    ...
    .then(medalsPage => {
        var medalsJSON = medalsPage.getCopy() as MedalsJSONFormat;
        medalsJSON.dataUser = {}
...
function syncMedalsListPageWithScript () {
    createPageFromTitle('Project:Medals').then(medalsPage => {
        ...
        .then(medalsListTemplate => {
            var content = medalsListTemplate.getCopy() as string;
            ...
            for (const medal of medals) {
                ...
                content = content.replace(
                    new RegExp(`\\[${medal.name} Title]`, 'g'),
                    medalDescription
                );
                content = content.replace(
                    new RegExp(`\\[${medal.name}]`, 'g'),
                    medalUsers === '' ? 'N/A' : `{{#invoke:String|join|${medalUsers}}}`
                );
            }
...
function addUsersToCSS () {
    function startFormat (medalName: string) {
        return '/* '+medalName+' */'
    }
    function endFormat (medalName: string) {
        return '/* END '+medalName+' */'
    }
    function ruleFormat (name: string) {
    ...
    createPageFromTitle('MediaWiki:Common.css').then(CommonCSS => {
        ...
        var content = CommonCSS.getCopy() as string;

        function addRulesToMedalCSS (medalName: string, rules: string) {
