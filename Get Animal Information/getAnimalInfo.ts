...
const wiki_tools = (function(){
    ...
    const ui_animals = (function self (){
        ...
        function createAnimal (petCardsListChild: HTMLDivElement) {
            // start client-side optimization
            optimizeCode.start();

            function createAttributeDetails (tableRow: HTMLTableRowElement) {
            ...
            for(var i = 1, l = statsTableChildren.length; i < l; i++) {
                const attributeDetails = createAttributeDetails(statsTableChildren[i] as HTMLTableRowElement);
